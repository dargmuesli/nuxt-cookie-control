...
export default(context, inject) =>{
  ...
  let methods = {
    ...
    set: ({name, value='', expires='', path='/', domain}) => {
      if(process.browser){
        document.cookie = `${name}=${value};expires=${expires};path=${path}${domain !== undefined && !isIE ? `;domain=${domain}` : ';'}`;
      } else if(process.server){
        context.res.setHeader('Set-Cookie', [`${name}=${value}; Expires=${expires}; Path=${path}${domain !== undefined && !isIE ? `; Domain=${domain}` : ';'}`]);
      }
    },
  ...
  if(cookies.consent === true){
    ...
    if(cookies.blockIframe && enabledFromCookie.includes('functional')) cookies.enabled.push({name: 'functional'});
