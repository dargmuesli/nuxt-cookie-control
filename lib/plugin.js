...
export default(context, inject) =>{
  ...
  const clearCookies = () =>{
    let disabled = cookies.optional.filter(c => {return !cookies.enabledList.includes(cookies.slugify(c.name))});
  ...
  if(cookies.consent === true){
    ...
    cookies.enabled.push(...cookies.optional.filter(c => {return enabledFromCookie.includes(cookies.slugify(c.name))}));
    cookies.enabledList = cookies.enabled.length > 0 ? cookies.enabled.map(c => {return cookies.slugify(c.name)}) : [];
  }
  
  cookies.enabled.push(...cookies.necessary.filter(c => {return c.src || c.accepted}))
